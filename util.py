...
if torch.cuda.is_available():
    available_gpus = get_available_gpus(min_free_memory_gb=70)
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pos, neg, pos_label, neg_label, pos_sim,pos_id,neg_id = self.get_pair(
            anchor_id, anchor_label)
        self.neg_sim = self.neg_sim +1
        return (anchor, pos, neg, anchor_label, pos_label, neg_label, pos_sim,anchor_id,pos_id,neg_id)
    ...
    def get_pair(self,  anchor_id, anchor_label, hardness_level=None, verbose=False):
        ...
        return pos, neg, pos_label, neg_label, pos_similarity,pos_id,neg_id
...
class DataSplitter():
    def __init__(self,data_dir, id2embedding,id2embedding_smile,n_classes=4, verbose=True,num_workers=12):
        self.verbose = verbose
        self.data_dir = data_dir
        self.n_classes = n_classes
        self.cath_label_path = self.data_dir / 'hierarchical-level.txt'
        self.id2embedding = id2embedding
        self.id2embedding_smile = id2embedding_smile
    ...
    def get_embedding_for_id(self, _id):
        try:
            embd_1 = torch.tensor(self.id2embedding[_id]).float()
            # embd_2 = torch.tensor(self.id2embedding_smile[_id]).float()
            embd_2= self.id2embedding_smile[_id]
            return _id, [embd_1,embd_2]
        except KeyError:
    ...
    def read_ids(self, path):
        ...
        id_list = [id_ for id_, emb in self.id2embedding.items()]
    ...
    def get_predef_splits(self, p_train=None, p_test=None):
        ...
        num_samples = int(len(train_keys) * 0.01)

        # Randomly sample the training keys for the validation lookup table
        sampled_keys = random.sample(train_keys, int(num_samples*0.5))
        
        val = {key: train[key] for key in sampled_keys}
...
class MyCollator(object):
    ...
    def __call__(self, batch):
        X_pos_neg, X_anchor, Y = [], [], []
        anchor_ids, pos_ids, neg_ids = [], [], []

        for (anchor, pos, neg, anchor_label, pos_label, neg_label, pos_sim, anchor_id, pos_id, neg_id) in batch:
            x = [pos[1], neg[1]]
            X_pos_neg.append(x)
            X_anchor.append(anchor[0].unsqueeze(0))
            Y.append(self.get_label_vector(anchor_label, pos_label, neg_label))
            anchor_ids.append(anchor_id)
            pos_ids.append(pos_id)
            neg_ids.append(neg_id)

        return (X_pos_neg, torch.cat(X_anchor, dim=0), torch.cat(Y, dim=0), anchor_ids, pos_ids, neg_ids)
...
class Eval():
    def __init__(self, lookup, test, datasplitter, n_classes, name='Eval'):
        ...
        self.test = torch.stack([i[0] for i in test.values()])
        self.lookup = torch.stack([i[0] for i in lookup.values()])
        # self.lookup = torch.tensor(np.array([i[0] for i in lookup.values()]))
    ...
    def evaluate(self, lookup, queries, n_nearest=1, update=True):
        ...
        if isinstance(lookup, torch.Tensor):
            lookup = lookup.detach().cpu().numpy().astype(np.float32)
        elif isinstance(lookup, np.ndarray) and lookup.dtype != np.float32:
        ...
        print(f"[DEBUG] FAISS search input shape: {queries.shape}")        
        
        distances, nn_idxs = self.index.search(queries, n_nearest)

        print(f"[DEBUG] FAISS distances input shape: {distances.shape}")        
...
def tokenize_dna(seq, max_len=2048):
    vocab = {'A': 0, 'T': 1, 'C': 2, 'G': 3, '[PAD]': 4}
    return [vocab.get(base, vocab['[PAD]']) for base in seq[:max_len]] + [vocab['[PAD]']] * (max_len - len(seq))
...
def testing(mdl, test, batch_size=30):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(test_emb), batch_size):
            ...
            test_tucker_batch = mdl.single_pass(batch)[0]
        ...
        for i in range(0, len(lookup_emb), batch_size):
            ...
            lookup_tucker_batch = mdl.single_pass(batch)[0]
